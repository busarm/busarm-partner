...
export class AddTripPage extends PageController {
    ...
    public async selectOrigin(event) {
        if (event.isTrusted) {
            if (this.userInfo) {
                this.selectLocation(this.userInfo.country, this.strings.getString('select_pickup_txt'), place => {
                    let location = this.processLocation(place);
                    if (location.country_code == this.userInfo.country.country_code || location.country == this.userInfo.country.country_name){
    ...
    public async selectDestination(event) {
        if (event.isTrusted) {
            if (this.userInfo) {
                this.selectLocation(this.userInfo.country, this.strings.getString('select_dropoff_txt'), place => {
                    let location = this.processLocation(place);
                    if (location.country_code == this.userInfo.country.country_code || location.country == this.userInfo.country.country_name){
    ...
    public async addTicket() {
        if (this.userInfo) {
            await this.showAddTicket(this.userInfo.country, ticket => {
                let found = false;
