...
export class AddLocationPage extends PageController {
  ...
  public async ngOnInit() {
    ...
    if (typeof google === "undefined" || !google) {
      if (
        this.session &&
        this.session.configs &&
        this.session.configs.google_api_key
      ) {
    ...
    if (event.isTrusted) {
      if (this.user) {
        this.searchLocation(
          ...
          this.strings.getString("select_location_txt"),
          (place) => {
  ...
  async searchLocation(
    ...
    let chooseModal = await this.modalCtrl.create({
      ...
      componentProps: {
        title: title,
        country: country,
        countries: this.session.countries,
      },
    });
  ...
  public submit() {
    ...
    this.selectedLocation.name = this.assertAvailable(this.selectedLocationName)
