...
export class ViewTripPage extends PageController {
    ...
    async showAddTicket() {
        let country = await new Promise<Country>((resolve:(country:Country)=>any,reject) => {
    ...
    /**Show Delete confirmation
     * */
    public confirmDeleteTicket(ticket:TicketInfo) {
        this.showAlert(
            this.strings.getString("delete_ticket_title_txt"),
            this.strings.getString("delete_ticket_msg_txt"),
            {
                title: this.strings.getString("no_txt")
            },
            {
                title: this.strings.getString("yes_txt"),
                callback: () => {
                    this.deleteTripTicket(ticket.ticket_id,ticket.type_id);
                }
            },
        );
    }
    ...
    /**Delete Trip Ticket*/
    public deleteTripTicket(ticketId: string,typeId: string) {
        this.showLoading().then(()=>{
            Api.deleteTicket(ticketId,typeId, (status, result) => {
                this.hideLoading();
                if (status) {
                    if (this.assertAvailable(result)) {
                        if (result.status){
    ...
    dismiss(){
        this.modalCtrl.dismiss(this.updated)
    }
}
