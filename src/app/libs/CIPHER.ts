...
export class CIPHER {
     ...
     * */
    static encrypt(passphrase: string, plain: string, callback: (status: boolean, result?: string) => any) {
        if (passphrase) {
            
            let salt = CryptoJS.lib.WordArray.random(256);
            let key = CryptoJS.PBKDF2(passphrase, salt, { hasher: CryptoJS.algo.SHA512, keySize: 64/8, iterations: 100 });

            if (this.platform.is('cordova') && this.aes256){
                this.aes256.generateSecureIV(key.toString())
                    .then(iv =>
                        this.aes256.encrypt(key.toString(), iv, plain)
                            .then(cipher => {
                                let ciphertext = CryptoJS.enc.Base64.stringify(cipher);
                                let hash = CIPHER.getDigest(ciphertext, CryptoJS.MD5(passphrase).toString());
                                let data = {
                                ...
                                callback(true, CryptoJS.enc.Base64.stringify(CryptoJS.enc.Utf8.parse(JSON.stringify(data))));
                            })
     ...
     * @param callback Action to perform after encryption
     * */
    static decrypt(passphrase: string, cipher: string, callback: (status: boolean, data?: string) => any) {
        if (passphrase) {
            try {
                let data = JSON.parse(CryptoJS.enc.Utf8.stringify(CryptoJS.enc.Base64.parse(cipher)));
                if(data){
                    ...
                    if(hash == CIPHER.getDigest(ciphertext, CryptoJS.MD5(passphrase).toString())){

                        let key = CryptoJS.PBKDF2(passphrase, salt, { hasher: CryptoJS.algo.SHA512, keySize: 64/8, iterations: 100 });

                        if (this.platform.is('cordova') && this.aes256){
                            this.aes256.decrypt(key.toString(), iv, CryptoJS.enc.Hex.stringify(CryptoJS.enc.Base64.parse(ciphertext)))
